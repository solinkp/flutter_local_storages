...
import 'package:flutter_local_storages/application/cbl/cbl_provider.dart';
...
class CblScreen extends ConsumerWidget {
  const CblScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    var cblDataAsync = ref.watch(cblCharsProvider);

    return BaseLayout(
      hasSyncAction: true,
      clearAction: () => ref.read(cblCharsProvider.notifier).cleanData(),
      syncAction: () => ref.read(cblCharsProvider.notifier).syncRemote(),
      title: 'CBL Data',
      body: cblDataAsync.when(
